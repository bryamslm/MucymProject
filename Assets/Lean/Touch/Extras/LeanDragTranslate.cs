	...
	public class LeanDragTranslate : MonoBehaviour
	{
		...
		private void Translate(Vector2 screenDelta)
		{
			...
			if (camera != null)
			{
				...
				// Convert back to world space
				screenPoint = camera.ScreenToWorldPoint(screenPoint);
				//screenPoint = new Vector3(screenPoint.x, 0, screenPoint.z);

				screenPoint = transform.parent.transform.InverseTransformPoint(screenPoint);
				screenPoint = new Vector3(screenPoint.x, -0.5f, screenPoint.z);

				Rigidbody rb = transform.GetComponent<Rigidbody>();
				rb.constraints = RigidbodyConstraints.FreezeRotationX | RigidbodyConstraints.FreezeRotationY | RigidbodyConstraints.FreezeRotationZ;

				Vector3 newPosition =  transform.parent.TransformPoint(screenPoint);
				//Debug.Log(newPosition);
				//rb.MovePosition(newPosition);
				//transform.localPosition = screenPoint;
				//Vector3 heading = (newPosition - transform.position).normalized;
				//rb.AddForce(heading * 1000 * Time.deltaTime);

				rb.velocity =   (newPosition - transform.position) * 10;
